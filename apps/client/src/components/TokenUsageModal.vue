    ...
    <div class="bg-background border rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
      ...
      <div v-else class="p-6 space-y-5">
        ...
        <div class="space-y-3">
          ...
          <!-- Modal-specific additional info -->
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            <Card class="p-3 text-center">
              <div class="text-sm font-bold" :class="burnRateColorClass">{{ formatNumber(currentBurnRate) }}</div>
              <div class="text-xs text-muted-foreground">Current Rate</div>
              <div class="text-xs text-muted-foreground opacity-75">tokens/min</div>
            </Card>
            <Card class="p-3 text-center">
              <div class="text-sm font-bold text-blue-600">{{ formatNumber(avgBurnRate) }}</div>
              <div class="text-xs text-muted-foreground">Average</div>
              <div class="text-xs text-muted-foreground opacity-75">session avg</div>
            </Card>
            <Card class="p-3 text-center">
              <div class="text-sm font-bold text-orange-600">{{ formatNumber(peakBurnRate) }}</div>
              <div class="text-xs text-muted-foreground">Peak Rate</div>
              <div class="text-xs text-muted-foreground opacity-75">session peak</div>
            </Card>
            <Card class="p-3 text-center">
              <div class="text-sm font-bold text-green-600">{{ activityHistory.length }}</div>
              <div class="text-xs text-muted-foreground">Data Points</div>
              <div class="text-xs text-muted-foreground opacity-75">live updates</div>
            </Card>
          </div>
        </div>
...
let targetBurnRate = 0
let currentAnimatedRate = 0
...
const drawGauge = () => {
  ...
  // Calculate angle based on burn rate (0 to 500K tokens/min scale for typical usage)
  const maxRate = 500000
  const normalizedRate = Math.min(currentAnimatedRate / maxRate, 1)
  const angle = Math.PI + (normalizedRate * Math.PI)
  
  // Gradient for active arc
  const gradient = ctx.createLinearGradient(0, 0, width, 0)
  if (normalizedRate < 0.3) {
...
// Draw sparkline with real-time updates
const drawSparkline = () => {
  ...
  const maxValue = Math.max(...activityHistory.value, 1)
  ...
  // Draw line
  ctx.beginPath()
  activityHistory.value.forEach((value, index) => {
    const x = (index / Math.max(activityHistory.value.length - 1, 1)) * width
  ...
  if (activityHistory.value.length > 0) {
    const lastValue = activityHistory.value[activityHistory.value.length - 1]
    const x = width
    const y = height - (lastValue / maxValue) * (height - 10) - 5
...
// Animate burn rate changes
const animateBurnRate = () => {
  const diff = targetBurnRate - currentAnimatedRate
  currentAnimatedRate += diff * 0.1
  
  if (Math.abs(diff) > 0.5) {
    drawGauge()
    animationFrame = requestAnimationFrame(animateBurnRate)
  } else {
    currentAnimatedRate = targetBurnRate
    drawGauge()
  }
}
...
const handleWebSocketMessage = (event: MessageEvent) => {
  try {
    ...
    if (data.type === 'tokenUsage' && data.data) {
      ...
      // Use ccusage's projection if available, otherwise calculate simple projection
      if (usage.projection) {
      ...
      } else if (usage.isActive && burnRate > 0 && tokenData.value) {
        ...
        const projectedTokens = tokenData.value.totalTokens + (burnRate * (remaining / 60000))
        const costPerToken = tokenData.value.costUSD / tokenData.value.totalTokens
        const projectedCost = tokenData.value.costUSD + ((burnRate * (remaining / 60000)) * costPerToken)
